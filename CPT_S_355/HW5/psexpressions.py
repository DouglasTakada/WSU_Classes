...
class Literal(Expr):
    ...
    def eval(self, ps_env):
        ...
        if isinstance(self.value, int) or isinstance(self.value, bool):
            ...
            print("Error in Literal eval method. value needs to be type int or bool")
...
class PSString(Expr):
    ...
    def eval(self, ps_env):
        if isinstance(self.value, str):
            ...
            print("Error in PSString eval method. value needs to be type str")
...
class PSName(Expr):
    ...
    def eval(self,ps_env):
        ...
        elif isinstance(ps_env.lookup(self.value), CodeArrayValue):

            index = ps_env.dictstack[-1][1][self.value]
            ps_env.lookup(self.value).apply(ps_env, index) 
            
        else:
            ps_env.opPush(ps_env.lookup(self.value))
...
class CodeArrayValue(Value):
    ...
    def apply(self, ps_env):
        ps_env.dictPush((ps_env.dictstack[-1][0], {}))
