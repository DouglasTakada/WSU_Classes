...
class PSOperators:
    def __init__(self,scoperule):
        if scoperule == "static":
            pass
        elif scoperule == "dynamic":
            pass
        else:
            print("Invaild scoping rule")
        #stack variables
        ...
        self.builtin_operators = {
            ...
            "search" : self.search,
            #Deleted end and begin operators since SSPS doesn't use them
            "def":self.psDef,
            "if":self.psIf,
    ...
    def dictPush(self,d):
        if isinstance(d,tuple):
            ...
            print("Error: dictPush - input value for this must be type tuple")
    ...
    def define(self,name, value):
        if len(self.dictstack) == 0:
            self.dictstack.append((0,{})) # pushing tuple of index and dictionary to dictstack
        self.dictstack[-1][1][name] = value # Acessing dictionary part of tuple
    ...
    def lookup(self,name):
        if self.scope == "dynamic":
            for i in range(len(self.dictstack)-1,-1,-1):
                if self.dictstack[i][1].get("/"+name) is not None:
                    return self.dictstack[i][1].get("/"+name)
            print("Could not find function or variable in dictstack for dynamic scoping")
            return None
        elif self.scope == "static":
            if "/"+name in self.dictstack[self.dictstack[-1][0]][1]: # go to static index and check if name is in the index
                return self.dictstack[self.dictstack[-1][0]][1]["/"+name]
            else:
                print("Error in lookup function. Static link does not point to value")
    ...
    def stack(self):
        ...
        for link,dic in reversed(self.dictstack):
            print(f"----Level: {size}----Link: {link}----\nDict: {dic}")
            size = size-1
        print("-----------------------"+ CEND)
        ...
        if len(self.opstack) > 2:
            ...
            if isinstance(newstr, StringValue) and isinstance(idx, int) and isinstance(value, StringValue):
                if idx+newstr.length() >= value.length():
                    value.value = value.value[:idx+1] + newstr
                else:
                    value.value = value.value[:idx+1] + str(newstr.value[1:-1]) + " " +value.value[idx+newstr.length():]
        else:
    ...
    def psIf(self):
        if len(self.opstack) > 1:
            ...
            if isinstance(v1, CodeArrayValue) and isinstance(v2, bool):
                if v2:
                    v1.apply(self,len(self.dictstack)-1)
            else:
    ...
    def psIfelse(self):
        if len(self.opstack) > 2:
            ...
            if isinstance(v1, CodeArrayValue) and isinstance(v2, CodeArrayValue) and isinstance(v3, bool):
                if v3:
                    v2.apply(self,len(self.dictstack)-1)
                else:
                    v1.apply(self,len(self.dictstack)-1)
            else:
