...
class Expr:
    ...
    In our interpreter, there are four types of expressions:
        ...
        2.	PSName: represents names of variables, functions, or operators .  The `var_name` attribute contains the name of the variable as a Python
            string, e.g., '/sq','sq','add','def'. If the `var_name` starts with a `/` character, PSName represents a name constant, otherwise it represents
            a variable reference ,  function call, or a built-in operator call. 
        3.	PSString: represents strings. The `value` attribute contains the string value the `PSString` refers to e.g., '(CptS355)'.
        4.	PSCodeArray: represents body of a function or if, ifelse, or for expressions. The `value` attribute is a Python list that includes the tokens 
            of the PostScript code-array (block) it represents [Literal(10), Literal(5),PSName(mul)]
...
class PSName(Expr):
    """A `PSName` is a variable , a built-in operator, or a function. 
        a.	If the `PSName` represents a name constant (i.e., its `var_name` attribute starts with a `/`),
            it will be evaluated to a Python string having value `var_name` . The evaluated value will be pushed onto the opstack.
        b.	If the `PSName` represents a built-in operator (i.e., its `var_name` attribute is one of the built-in operator names),
    ...
    def eval(self,ps_env):
        ...
        if self.var_name[0] =='/':
            ...
            #look up name in dict stack
            tmp = ps_env.lookup(self.var_name)
            if tmp == None:
                print("Tried to look up variable name that's not in dict stack")
            else:
                if isinstance(tmp,CodeArrayValue):
                    tmp.apply(ps_env)
                else:
                    ps_env.opPush(tmp)
...
class Value:
    ...
    In our interpreter,
        ...
        -  StringValue, DictionaryValue, and CodeArrayValue implement the following methods in the `Value` interface:
            * apply : Evaluates the value. `apply` is only applicable to CodeArrayValue objects (applies the function, evaluates all the tokens
            in the function's code-array, i.e., CodeArrayValue )  
